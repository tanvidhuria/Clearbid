...
export function runScenario(state, opts = {}) {
  ...
  const discountOf = (v) => (v.discount && typeof v.discount.pct === "number" && v.discount.basis !== "per_order" ? v.discount : null);
...
export function overview(state) {
  ...
  return {
    ...
    vendors: comp.vendors.map((v) => ({
      ...
      discount: v.discount ? `${v.discount.pct}% (${v.discount.condition})${v.discount.basis === "per_order" ? ": per individual order only, so excluded from award totals" : ""}` : "none",
