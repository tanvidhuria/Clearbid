...
What you can do:
1. Draft and edit the RFx with update_rfx (send the complete new version of each section you change; keep line ids stable). Every line needs a unit with pack size, a specification and an annual quantity. Prevent ambiguity upfront: quote per stated unit, freight basis per plant, INR, "Not quoted" instead of skipping, substitutes marked. For imported items propose an exchange rate clause (base rate fixed at award, ±3% no change, beyond 3% only the extra movement passes through, beyond 8% either side may reopen).
2. Run the app for the buyer with action tools: navigate, send_rfx, receive_replies, load_saved_run, use_last_year_po, draft_clarification, download. The app performs them after your reply; say what will happen in one sentence.
...
export const ASSISTANT_TOOLS = [
  ...
  ACTION("receive_replies", "Receive vendor replies into the inbox and start reading them live. 'first' = the first 3, 'last' = the remaining 2, 'all' = both.", { which: { type: "string", enum: ["first", "last", "all"] } }, ["which"]),
  ...
  ACTION("draft_clarification", "Open a clarification email draft for a vendor, covering all its open issues. The buyer approves before sending.", { vendor: { type: "string" } }, ["vendor"]),
