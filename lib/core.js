...
function vendorIssues(v, rfx, cells, info, t) {
  ...
  if (t.discount && typeof t.discount.pct === "number" && t.discount.basis === "per_order") add("low", `Discount ${t.discount.pct}% applies only to a single purchase order above ${t.discount.threshold_inr ? "₹" + Number(t.discount.threshold_inr).toLocaleString("en-IN") : "the threshold"}; monthly call-off orders are smaller, so it is left out of award totals`, null);
  else if (t.discount && typeof t.discount.pct === "number") add("medium", `Discount ${t.discount.pct}% with condition: ${t.discount.condition}`,
