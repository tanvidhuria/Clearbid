...
export async function readVendor(rfx, vendor, onStep) {
  ...
  const follow = vendor.files.filter((f) => f.role === "followup");
  const quoteSet = await Promise.all([...email, ...(quoteFiles.length ? quoteFiles : attach), ...follow].map(load));
  const allSet = await Promise.all([...email, ...quoteFiles, ...attach, ...follow].map(load));
